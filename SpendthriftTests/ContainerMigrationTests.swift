...
/// `SpendthriftContainer.migrateIfNeeded` — the one-time copy of the legacy
...
struct ContainerMigrationTests {
    ...
    private func makeTempStoreURLs() throws -> (root: URL, old: URL, new: URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("spendthrift-migration-\(UUID().uuidString)")
        ...
        return (
            ...
            newDir.appendingPathComponent("Spendthrift.store")
        )
    }
    ...
    @Test func copiesStoreAndSidecarsToNewLocation() throws {
        ...
        try SpendthriftContainer.migrateIfNeeded(from: old, to: new)
