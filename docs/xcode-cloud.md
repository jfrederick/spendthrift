...
- [ ] Register the **App Group** (`group.dev.jimfrederick.spendthrift`) and the
      **widget extension bundle id** (`dev.jimfrederick.spendthrift.widgets`) in
      the Apple Developer portal / App Store Connect, and confirm both the
      app and `SpendthriftWidgets` targets are signed with the App Group
